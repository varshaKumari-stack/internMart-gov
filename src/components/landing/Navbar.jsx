      ...
      <div className="relative mx-auto flex h-20 items-center justify-between px-4 md:px-6">
        ...
        <Link to="/" className="group flex items-center gap-4">
          ...
          <motion.div
            ...
            className="relative flex h-12 w-12 items-center font-[CYR] justify-center rounded-2xl bg-gradient-to-r from-pink-500 to-red-500 text-xl font-black shadow-[0_0_35px_rgba(255,0,80,0.45)]"
          >
            G
          </motion.div>

          <div>
            <h1 className="bg-gradient-to-r font-[CYR]  from-white via-pink-200 to-cyan-200 bg-clip-text text-2xl font-black text-transparent">
              GovMart India
            </h1>

            <p className="text-xs uppercase tracking-[0.3em]font-[PG]  text-zinc-500">
        ...
        <nav className="hidden lg:flex items-center gap-3 rounded-full border border-white/10 bg-white/[0.03] px-5 py-2 backdrop-blur-2xl">
          ...
          {navItems.map((item, i) => (
            ...
            >
              <Link
                to={item.path}
                className="relative rounded-xl px-4 py-2 text-sm font-[AEO-REG] uppercase tracking-[0.18em] text-zinc-300 transition duration-300 hover:bg-white/5 hover:text-pink-400"
              >
                {item.name}
              </Link>
            </motion.div>
        ...
        <div className="flex items-center gap-3">
          ...
          <motion.div whileHover={{ scale: 1.05 }}>
            <Link
              to="/products"
              className="group relative  font-[AEO-REG]  hidden md:flex items-center gap-2 overflow-hidden rounded-2xl border border-white/10 bg-white/5 px-5 py-3 text-sm font-semibold text-zinc-300 backdrop-blur-xl"
            >
              <Search size={16} />
              Search Tourism
            </Link>
